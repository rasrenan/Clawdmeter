...
async def poll_api(token: str) -> dict | None:
    ...
    # Auth errors carry no such headers, so those still bail (above).
...
async def poll_active(selector: PlanSelector = _SELECTOR) -> tuple[dict | None, bool]:
    ...
    for d in dirs:
        ...
        if not token:
            ...
            continue
        if token_expired(d):
            # Known-expired locally: skip without burning an API call. Not
            # "live" either, so the no-data beat still fires.
            continue
        try:
        ...
        while client.is_connected and not stop_event.is_set():
            ...
            if due:
                ...
                if payload is not None:
                    ...
                    # cycle) -> stay silent and retry next cycle.
